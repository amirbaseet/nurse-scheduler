...
export function CreateClinicDialog({
  ...
  function resetForm() {
    ...
    setCode("");
    setGenderPref("ANY");
    setCanBeSecondary(false);
    setSecondaryHours("0");
    setSecondaryNursesNeeded("0");
    setError(null);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409) {
          throw new Error(t("code_already_exists"));
        }
        const data = await res.json().catch(() => null);
        throw new Error(data?.error ?? t("clinic_create_error"));
      }
      ...
      setError(
        err instanceof Error ? err.message : t("clinic_create_error"),
      );
    } finally {
      setSaving(false);
    }
    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{t("add_clinic")}</DialogDescription>
        </DialogHeader>
